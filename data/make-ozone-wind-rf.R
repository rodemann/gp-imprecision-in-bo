...
parameter_set = makeParamSet(
  ...
  makeNumericParam("Wind_speed", lower = cov_range[1], upper = cov_range[2])#,
...
grid <- data.frame("Wind_speed" = seq(cov_range[1], cov_range[2], length.out = 10000))
...
names(ozone_hum_df) = c("Wind_speed", "ozone_reading")
graph_ozone_hum_plot =  qplot(data = ozone_hum_df, x=ozone_hum_df$"Wind_speed", y=ozone_hum_df$"ozone_reading", 
                         geom = "line", xlab = "Wind_speed", ylab = "ozone_reading",
                         main = "Ozone-Wind_speed target function (Random Forest)")
