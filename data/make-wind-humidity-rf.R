...
Ozone = select(Ozone, V5, V7)
Ozone = tidyr::drop_na(Ozone)
cov_range = c(min(Ozone[,2]), max(Ozone[,2]))
names(Ozone) = c("pressure_height", "Humidity")
ground_truth = train(makeLearner("regr.randomForest"), 
                     makeRegrTask(data = Ozone, target = "pressure_height"))

fun = function(x) {
  df = data.frame("Humidity" = x)
...
parameter_set = makeParamSet(
  ...
  makeNumericParam("Humidity", lower = cov_range[1], upper = cov_range[2])#,
...
names(ozone_hum_df) = c("Humidity", "pressure_height")
graph_ozone_hum_plot =  qplot(data = ozone_hum_df, x=ozone_hum_df$"Humidity", y=ozone_hum_df$"pressure_height", 
                              geom = "line", xlab = "Humidity", ylab = "500 millibar pressure height",
                              main = "pressure height-humidity target function (Random Forest)")
