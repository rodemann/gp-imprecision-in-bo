...
parameter_set = makeParamSet(
  ...
  makeNumericParam("Pressure", lower = cov_range[1], upper = cov_range[2])#,
...
grid <- data.frame("Pressure" = seq(cov_range[1], cov_range[2], length.out = 10000))
...
names(ozone_hum_df) = c("Pressure", "ozone_reading")
graph_ozone_hum_plot =  qplot(data = ozone_hum_df, x=ozone_hum_df$"Pressure", y=ozone_hum_df$"ozone_reading", 
                              geom = "line", xlab = "Pressure", ylab = "ozone_reading",
                              main = "Ozone-Pressure target function (Random Forest)")
