...
load("imp-BO_benchmarking/results/results-imp-BO-synthetic-glcb-drop-wave")
...
plots_lcb <- plot #+ xlim(0,40) #+ theme_update() + scale_color_manual(values = cbbPalette)   #+scale_color_manual()
...
plots_ei <- plot #+ xlim(0,40) #+ theme_update() + scale_color_manual(values = cbbPalette)   #+scale_color_manual()
...
annotate_figure(pub_page,
                top = text_grob("GLCB vs. EI and LCB (Drop Wave)", color = "black", face = "bold", size = 14),
                bottom = text_grob("Bayesian optimization of Drop Wave Function
60 runs per Acquisition Function with 60 evaluations and initial sample size 10 each
