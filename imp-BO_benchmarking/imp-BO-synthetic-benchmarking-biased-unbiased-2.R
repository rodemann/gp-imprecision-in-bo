...
# divide into chunks
synth_test_functions <- all_benchmark_funs[1]
...
for (i in 1:2) {
  ...
  for (j in 1:n) {
    ...
    if(i == 2){ # biased design
      parameter_set_biased = makeParamSet(makeNumericParam("x", lower = -30, upper = -20))
...
save(results_list, file = paste(getwd(),"/imp-BO_benchmarking/results/results-imp-BO-tests-init-4-budget-40-n-40-ackley-biased" ,sep=""))
