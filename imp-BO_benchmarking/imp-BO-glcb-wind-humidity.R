...
source("data/make-wind-humidity-rf.R") 
...
fun <- ozone_fun_rf #it's wind-humidity actually
...
for (i in 1:1) {
  ...
  for (j in 1:n) {
    ...
    ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 100, cb.rho = 10, base_kernel = "powexp")) 
    glcb_20_250_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)
    ...
    opt_paths_raw <- list(getOptPathY(cl_bo_res_lcb[["opt.path"]]), getOptPathY(cl_bo_res_alcb[["opt.path"]]),
                          ...
                          getOptPathY(glcb_20_250_bo_res[["opt.path"]]))
...
save(results_list, file = paste(getwd(),"/imp-BO_benchmarking/results/results-imp-BO-wind-humidity" ,sep=""))
