...
source("data/make-kapton-rf.R") #feature: time
...
for (i in 1:2) {
  ...
  for (j in 1:n) {
    ...
    # glcb with lambda = 1, rho = 2, imp = 100
    ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 100, cb.rho = 2, base_kernel = "powexp")) 
    glcb_2_100_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)
    ...
    # glcb with lambda = 1, rho = 2, imp = 50
    ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 50, cb.rho = 2, base_kernel = "powexp")) 
    glcb_2_50_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)
    ...
    # glcb with lambda = 1, rho = 1, imp = 10
    ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 10, cb.rho = 1, base_kernel = "powexp")) 
    glcb_1_10_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)
    
    
    
    
    opt_paths_raw <- list(getOptPathY(cl_bo_res_lcb[["opt.path"]]), getOptPathY(cl_bo_res_ei[["opt.path"]]), 
                          getOptPathY(glcb_1_100_bo_res[["opt.path"]]), getOptPathY(glcb_2_100_bo_res[["opt.path"]]),
                          getOptPathY(glcb_5_100_bo_res[["opt.path"]]), getOptPathY(glcb_1_50_bo_res[["opt.path"]]),
                          getOptPathY(glcb_2_50_bo_res[["opt.path"]]), getOptPathY(glcb_5_50_bo_res[["opt.path"]]),
                          getOptPathY(glcb_1_10_bo_res[["opt.path"]])
                          )
...
save(results_list, file = paste(getwd(),"/imp-BO_benchmarking/results/results-imp-BO-kapton-glcb-unbiased-and-biased-2" ,sep=""))
