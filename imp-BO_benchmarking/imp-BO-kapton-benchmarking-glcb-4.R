...
for (i in 1:1) {
  ...
  for (j in 1:n) {
    ...
    ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 10, cb.rho = 1, base_kernel = "powexp")) 
    glcb_10_100_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)

      ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 20, , cb.rho = 1, base_kernel = "powexp")) 
    ...
    glcb_10_50_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)
        ...
        ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 200, cb.rho = 1, base_kernel = "powexp")) 
    glcb_50_50_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)

        ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 500, cb.rho = 1, base_kernel = "powexp")) 
...
save(results_list, file = paste(getwd(),"/imp-BO_benchmarking/results/results-imp-BO-kapton-glcb-4" ,sep=""))
