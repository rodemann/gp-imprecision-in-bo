...
for (i in 1:1) {
  ...
  for (j in 1:n) {
    ...
    glcb_10_100_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)
      ...
      ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 100, , cb.rho = 50, base_kernel = "powexp")) 
    glcb_50_100_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)
    ...
    glcb_1_50_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)
        ...
        ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 50, cb.rho = 10, base_kernel = "powexp")) 
    glcb_10_50_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)

        ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 50, cb.rho = 50, base_kernel = "powexp")) 
    glcb_50_50_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)

        ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritGLCB(imprecision = 200, cb.rho = 10, base_kernel = "powexp")) 
    glcb_10_200_bo_res <- mbo(fun, design, ctrl, learner = lrn_classic, show.info = FALSE)
    
    
    
    
    opt_paths_raw <- list(getOptPathY(cl_bo_res_lcb[["opt.path"]]), getOptPathY(cl_bo_res_ei[["opt.path"]]), 
                          getOptPathY(glcb_10_100_bo_res[["opt.path"]]), getOptPathY(glcb_50_100_bo_res[["opt.path"]]),
                          getOptPathY(glcb_1_50_bo_res[["opt.path"]]), getOptPathY(glcb_10_50_bo_res[["opt.path"]]),
                          getOptPathY(glcb_50_50_bo_res[["opt.path"]]), getOptPathY(glcb_10_200_bo_res[["opt.path"]]))
...
save(results_list, file = paste(getwd(),"/imp-BO_benchmarking/results/results-imp-BO-kapton-glcb-3" ,sep=""))
